...
export default function DashboardLayout({ children, role }) {
  ...
  const NavLink = ({ to, icon, children }) => (
    <Link
      ...
      className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all duration-300 group relative ${
        isActive(to)
          ? "bg-green-600 dark:bg-emerald-600 text-white shadow-lg shadow-green-100 dark:shadow-emerald-900/50"
          : "text-gray-500 dark:text-slate-400 hover:bg-green-50 dark:hover:bg-emerald-950/30 hover:text-green-600 dark:hover:text-emerald-400"
      }`}
    >
      ...
      {isCollapsed && (
        <span className="absolute left-16 scale-0 group-hover:scale-100 transition-all bg-gray-800 dark:bg-slate-700 text-white text-[10px] px-2 py-1 rounded-md font-bold uppercase tracking-widest z-50">
          {children}
        </span>
      )}
    </Link>
  ...
  return (
    <div className="flex min-h-screen bg-[#FBFCFE] dark:bg-slate-950">
      {/* Overlay Mobile */}
      {isMobileOpen && (
      ...
      <div className="w-10 h-10 bg-green-600 rounded-2xl flex items-center justify-center shadow-lg shadow-green-200 dark:shadow-emerald-900/50 shrink-0">
      ...
      {!isCollapsed && (
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto no-scrollbar">
          ...
          <div className="pt-6 mt-6 border-t border-gray-50 dark:border-slate-800 space-y-2">
             {!isCollapsed && <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-300 dark:text-slate-600 mb-4 px-4">Manajemen</p>}
        ...
        <button onClick={() => setIsCollapsed(!isCollapsed)} className="hidden lg:flex items-center justify-center h-12 w-12 mx-auto mb-6 bg-gray-50 dark:bg-slate-800 text-gray-400 dark:text-slate-600 rounded-xl hover:text-green-600 dark:hover:text-emerald-400 transition-colors">
      ...
      <main className={`flex-1 flex flex-col min-h-screen transition-all duration-500 ${isCollapsed ? "lg:ml-20" : "lg:ml-72"}`}>
        ...
        <header className="sticky top-0 z-50 flex justify-between items-center bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl px-4 lg:px-8 py-4 border-b border-gray-100 dark:border-slate-800">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              <h1 className="text-[10px] font-black text-gray-400 dark:text-slate-600 uppercase tracking-widest leading-none">{role} System</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-3 bg-gray-50 dark:bg-slate-800 p-1.5 pr-4 rounded-full border border-gray-100 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 hover:shadow-md dark:hover:shadow-slate-900/50 transition-all"
            >
              {/* Avatar Bulat */}
            ...
            {showProfile && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl dark:shadow-slate-900/50 border border-gray-100 dark:border-slate-700 p-2 z-20 animate-in fade-in zoom-in duration-200">
                  <Link 
                    to={role === "petugas" ? "/profil-petugas" : "/profil-admin"}
                    onClick={() => setShowProfile(false)}
                    className="w-full text-left px-4 py-3 text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/50 rounded-xl flex items-center gap-3 transition-all"
                  >
                  ...
                  <hr className="my-1 border-gray-50 dark:border-slate-700" />
                  
                  <button 
                    onClick={handleLogout}
                    className="w-full text-left px-4 py-3 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-xl flex items-center gap-3 transition-all"
                  >
        ...
        <div className="p-4 lg:p-8">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm dark:shadow-slate-900/50 border border-gray-50 dark:border-slate-800 min-h-[calc(100vh-140px)] p-6 lg:p-10">
            {children}
          </div>
        </div>
      </main>
